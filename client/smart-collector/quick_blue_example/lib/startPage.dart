...
class _startPageState extends State<startPage> {
  ...
  Future<void> loadConfigVariables() async {
    ...
    //widget.globals.receivedIMUJsonValues = receivedIMUJsonValuesString != null
       ...
       // : [];
     widget.globals.receivedOrientationJsonValues = [];
  ...
  void _handleValueChange(String deviceId, String characteristicId, Uint8List value) {
    ...
    if(characteristicId == "8e7c2dae-0004-4b0d-b516-f525649c49ca"){ //ORIENTATION
      Map<String, dynamic>jsonObj = parseOrientationData(value, 3);
      setState(() {
         widget.globals.receivedOrientationJsonValues.add(jsonObj);
      });
    }
  ...
  Map<String, dynamic>parseOrientationData(Uint8List value, int ArrayLength) {
    final byteData = ByteData.view(value.buffer);
    List<double> orientationData = List<double>.filled(3, 0.0);

    double heading = byteData.getFloat32(0, Endian.little);  
    double pitch = byteData.getFloat32(4, Endian.little);
    double roll = byteData.getFloat32(8, Endian.little);

    orientationData[0] = heading;
    orientationData[1] = pitch;
    orientationData[2] = roll;

      Map<String, dynamic> jsonObj = {
        "timestamp": DateTime.now().millisecondsSinceEpoch,
        "values": orientationData,
      };
